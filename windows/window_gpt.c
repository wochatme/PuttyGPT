...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    ...
    AR_Init(inst); /* AskRob Initialization */
     ...
     */
    {
        ...
        for (j = 0; j < lenof(popup_menus); j++) {
            ...
            AppendMenu(m, MF_ENABLED, IDM_ASKROB, "Ask Rob"); /* AskRob */
            sfree(str);
        }
    }
    ...
    /* create the main window of AskRob */
    if (AskRobIsGood)
    {
        hWndPutty = wgs.term_hwnd;
        if (g_AskRobAtStartUp)
        {
            RECT puttyPosition;
            if (GetWindowRect(wgs.term_hwnd, &puttyPosition) == FALSE)
            {
                puttyPosition.left = 0;
                puttyPosition.top = 0;
                puttyPosition.right = 0;
                puttyPosition.bottom = 0;
            }
            StartupAskRob_UI_Thread(&puttyPosition);
        }
    }
        ...
        switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
          ...
          case IDM_RESET:
            ...
            if (ldisc)
                ldisc_echoedit_update(ldisc);
            break;
          case IDM_COPYSCREEN: /* AskRob */
            AR_CopyScreen(term, clips_system, lenof(clips_system));
            break;
          case IDM_ASKROB:  /* AskRob */
              if (AskRobIsGood)
              {
                  if (IsWindow(hWndAskRob))
                      PostMessage(hWndAskRob, WM_BRING_TO_FRONT, 0, 0);
                  else
                      StartupAskRob_UI_Thread(NULL);
              }
              break;
          case IDM_ABOUT:
            showabout(hwnd);
